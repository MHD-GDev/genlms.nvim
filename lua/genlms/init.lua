...
local function create_lualine_component()
    ...
    return function() return status end
end
...
local default_options = {
    ...
    command = function(options)
        local check = vim.fn.system("curl -s http://" .. options.host .. ":" ..
...
local function get_window_options(opts)
    ...
    -- Calculate window size as half of screen
    local win_width = screen_width
    local win_height = math.floor(screen_height * 0.5)
...
local function chunk_large_content(content, chunk_size)
    ...
    for i = 1, length, chunk_size do
        table.insert(chunks, string.sub(content, i,
                                        math.min(i + chunk_size - 1, length)))
    end
    return chunks
end
...
M.exec = function(options)
    ...
    if string.find(cmd, "%$body") then
        ...
        if globals.context then
            -- Only use the existing context if provided
            messages = opts.messages or globals.context
        end
...
function Process_response(str, json_response)
    ...
    if json_response then
        ...
        if success then
            if result.message and result.message.content then -- lms chat endpoint
                ...
                if #globals.context > 6 then
                    -- Remove older messages, keeping last 6
                    local new_context = {}
                    for i = #globals.context - 5, #globals.context do
                        table.insert(new_context, globals.context[i])
                    end
                    globals.context = new_context
                end
            ...
            elseif result.choices then -- LMStudio/OpenAI format
                ...
                if content ~= nil then
                    globals.context = globals.context or {}
                    -- Keep the last two user questions and AI response as context
                    if #globals.context > 3 then
                        globals.context = {
                            globals.context[#globals.context - 2], -- second last user question
                            globals.context[#globals.context - 1], -- last user question
                            globals.context[#globals.context] -- last AI response
                        }
                    end
                    globals.context_buffer = globals.context_buffer or ""
                    globals.context_buffer = globals.context_buffer .. content
